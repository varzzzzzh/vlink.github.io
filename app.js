...
async function startSmsHandover(data, isImage = false) {
  ...
  for (let i = 0; i < encrypted.length; i += chunkSize) {
    packets.push(
      `VLINK|ID:${tId}|SEQ:${Math.floor(i / chunkSize) + 1}|TOT:${Math.ceil(encrypted.length / chunkSize)}|DATA:${encrypted.substring(i, i + chunkSize)}`,
    );
  }
...
async function processIncoming(rawData) {
  ...
  if (!password) {
    addMessage(
      "Incoming packet detected. Enter passcode to decrypt.",
      "system",
    );
    return;
  }
...
fileInput.onchange = (e) => {
  ...
  addMessage("Processing color image for Student View...", "system");
  ...
  reader.onload = (event) => {
    ...
    img.onload = () => {
      ...
      // 1. SET DIMENSIONS
      // 600px is wide enough to read textbook text clearly
      const MAX_WIDTH = 600;
      const scaleSize = MAX_WIDTH / img.width;
      canvas.width = MAX_WIDTH;
      canvas.height = img.height * scaleSize;
      ...
      // 2. THE FIX: Change 'grayscale(1)' to 'none'
      ctx.filter = "none";

      // 3. DRAW & COMPRESS
      ctx.drawImage(img, 0, 0, canvas.width, canvas.height);

      // 0.3 Quality is the "Sweet Spot":
      // It keeps colors and sharp text but stays under your 10-SMS limit.
...
importInput.addEventListener("input", (e) => {
  if (e.target.value.startsWith("VLINK|"))
    processIncoming(e.target.value.trim());
});
...
document.getElementById("show-qr-btn").onclick = () => {
  ...
  if (qr.innerHTML === "")
    new QRCode(qr, { text: window.location.href, width: 128, height: 128 });
};
...
document.getElementById("reset-receiver").onclick = () => {
  if (
    confirm("DANGER: Wipe all friends and settings? This cannot be undone.")
  ) {
    localStorage.clear();
